/* ============================================================
FENRYS BOT — SIMPLE BOT STORE ESM — SMSG FIX JID (PURE PN ONLY)
Creator: Juna | 2025
============================================================ */
...
if (m.isGroup) {
const pn = pickPnStrict(m.key?.participant, m.key?.participantAlt)
m.sender = pn || ''
m.participant = pn || ''
} else {
const pn = pickPnStrict(m.key?.remoteJid, m.key?.remoteJidAlt)
m.sender = pn || ''
}
}
...
m.mentionedJid = mapMentionsToPn(ctx.mentionedJid)
...
// ambil LID asli dari contextInfo
const quotedLid = conn.decodeJid(ctx.participant || '')

// SET DEFAULT DULU (PASTI ADA, BIAR GA UNDEFINED)
m.quoted.sender = quotedLid || ''

// kalau grup & pakai LID → resolve ke phone number (NON-AWAIT)
if (m.isGroup && quotedLid && quotedLid.endsWith('@lid')) {
  conn.groupMetadata(m.chat).then(meta => {
    const found = meta?.participants?.find(p => p.id === quotedLid)
    const pn = found?.phoneNumber || found?.jid
...
m.quoted.mentionedJid = mapMentionsToPn(ctx.mentionedJid)

m.getQuotedObj = m.getQuotedMessage = async () => {
if (!m.quoted.id) return false
let qmsg = await store.loadMessage(m.chat, m.quoted.id, conn)
return smsg(conn, qmsg, store)
}
